...
- [ ] **PRE-FRONTEND GATE:** Schema alignment audit — verify all binary-decoded fields have corresponding DB columns and correct types; verify enum code-to-seed ID mappings are consistent; verify writer field-flow (every parser dict key consumed by insert_*); verify CHECK constraints accept all observed binary values. **Also:** resolve bonuses deduplication — with FID→stat lookup, binary Pass A and wiki Pass B may insert the same stat bonus twice. Need either: (a) unique constraint on (source_type, source_id, stat_id, bonus_type_id, value), or (b) Pass B skip logic for stats already resolved by Pass A, or (c) INSERT OR REPLACE keyed on semantic content. Goal: ensure no data is silently dropped, rejected, or duplicated.
