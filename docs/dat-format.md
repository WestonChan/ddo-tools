...
- [ ] Item numeric field correlation — re-run dat-namemap with full wiki catalog; check for enhancement_bonus, armor_bonus, max_dex_bonus, hardness, weight, base_value (items.py _WIKI_ONLY_FIELDS may be overly pessimistic)
- [ ] Item enum field correlation — search for binary property keys encoding material, binding, handedness, proficiency as small int enums; weapon_type as 0x0A string ref
- [ ] Spell field correlation — extend spells_correlate.py with correlators for components (bitmask), range (enum), saving_throw (enum), target (enum), duration (formula), spell_resistance (bool), damage_types (enum), metamagics (bitmask); 30+ unexplored ref slots per 0x47 entry
...
- [ ] Feat boolean flag discovery — key 0x1000088E ("unknown_bitmask", 32 distinct values) is a strong candidate for is_passive/is_active/is_stance/is_metamagic bitmask; correlate against wiki feat flags
