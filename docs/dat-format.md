...
- [ ] Linked chain property structure — **much larger than documented**: 6,372 entries have chain keys (was 78). Entries include "Elf", "Warforged", "Set Bonus", "Burning Hands" — chains span races, sets, and spells. Chain_type values 15/18/19 categorize the chain. **Needs:** decode chain traversal logic and determine what relationships they encode (set membership? enhancement prereqs? spell-to-class links?). The 6,372-entry scope makes this potentially high-value.
