...
- [ ] Effect type 414 decoding — **new effect type discovered** in effect_ref chains. 425-byte entries with embedded localization text (e.g., "Improved Critical: Great Crossbow" with description). May encode weapon special properties, feat-granted bonuses, or other complex effects. Needs: (a) census of all type-414 entries, (b) decode the 425-byte layout, (c) determine what build-relevant data they carry beyond the description text.
- [ ] Effect_ref slot-to-property mapping — while individual chains are traced, need systematic analysis: do specific effect_ref slots (e.g., effect_ref vs effect_ref_2 vs effect_ref_3) consistently encode specific property categories? Do augmented items have different effect chains than non-augmented? Do weapon-type effects always appear in a specific slot?
...
- [x] Runearm sub-schema — **decoded 3 keys**: 0x1000076B = tier/charge level (values 1/2/4), 0x10000C90 = charge rate float (0.05/0.15/0.5 — 5%/15%/50%), 0x1000278E = mode flag (0 or -2). Tier correlates with charge rate. **Issue:** equipment_slot=17 filter matches non-runearm entries ("Gorehoof", "Conjured Spider"). Need to verify correct equipment_slot enum value for Runearm before wiring. Keys are decoded but not yet wired into parser.
