...
- [x] **PRE-FRONTEND GATE:** Unimplemented discoveries audit (2026-04-10) — all 6 known items reviewed:
  - Feat active/stance/free flags: WIRED (binary -> wiki field overlay in cli.py). No dedicated columns needed.
  - Spell school hash lookup: WIRED (114-entry hash at ref slots 15/16, overlays school_id).
  - tick_count: WIRED (stat 731 -> spells.tick_count column).
  - Runearm keys: FALSE POSITIVE (generic tier/rate keys on trap parts, not runearms). Closed.
  - effect_ref slot mapping: DOCUMENTED but deferred. Type-167 localization provides values instead.
  - item_type_bitmask (0x1000088E): DISCOVERED (92% accuracy) but deferred. Wiki item_category is reliable.
  - **Conclusion:** No unimplemented discoveries block the frontend. All player-facing binary data is wired.
...
- [x] **PRE-FRONTEND GATE:** Binary vs wiki accuracy audit (2026-04-10) — wiki is dominant source for all entities. Binary supplements ~60 items (0.6%) and ~293 feats (36%). No accuracy conflicts (overlay only fills NULL fields). Key per-field results:
  - Items: item_category 100%, material 100%, weight 97%, minimum_level 93% from binary. equipment_slot 32% (wiki slot categories better). rarity/cooldown/durability 0% (not populating).
  - Feats: Boolean flags (is_active/passive/stance/metamagic/free) 100% from binary. cooldown/duration/damage sparse (1-5%).
  - Spells: school_id 98%, spell_points 99% from wiki+binary. tick_count 6% (DoT/HoT only).
  - Bonuses: 100% wiki for item_bonuses/item_effects/enhancement_bonuses. 2% binary for augment_bonuses.
  - **Conclusion:** No binary-derived values corrupt the DB. Wiki always wins when present. Binary is a marginal supplement.
- [x] **PRE-FRONTEND GATE:** Lookup table audit (2026-04-10) — 7 lookup tables reviewed (EFFECT_FID_LOOKUP 97 entries, fid_item_lookup.json 793, fid_enhancement_lookup.json 1,203, STAT_DEF_IDS 10, DISCOVERED_KEYS 442, school_hash 114). Results:
  - (a) EFFECT_FID_LOOKUP and fid_item_lookup cannot merge (different namespaces: 0x70 effects vs 0x10 properties).
  - (b) No duplicate or overlapping entries across tables.
  - (c) Multi-field fid_item_lookup entries are fine (lookup is by FID, not field).
  - (d) school_hash in cli.py could move to fid_lookups.py (minor cleanup, low priority).
  - (e) No lookup data belongs in DB seed (all are binary-format constants from RE, not game data).
  - **Conclusion:** Well-organized. No structural issues. One minor cleanup opportunity (school_hash location).
