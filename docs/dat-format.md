...
- [ ] Feat boolean flag discovery — key 0x1000088E has 8,318 entries with 144 unique values (richer taxonomy than expected). Wiki cross-ref with 61 matched feats showed no clean correlation with passive/active/stance/metamagic flags (stance and metamagic were 0% everywhere). Needs deeper analysis with more matches or a different approach (e.g., bit-level decomposition of the multi-byte values)
