...
- [ ] Augment slot format investigation — **wiki parser bug**: all 8,600 wiki items have `augment_slots=[]` (empty). Raw wiki fetch also shows empty augmentslot field — the `{{Named item}}` template parser may need fixing for `augmentslot` field extraction. No multi-occurrence keys found on items. **Candidate key: 0x10000A1D** has values 1-8 on 3,655 items — DDO has 8 augment slot colors, distribution plausible. **Blocked on:** fixing wiki augmentslot parser to enable cross-reference. Once wiki data is available, correlate key 0x10000A1D values against slot colors.
