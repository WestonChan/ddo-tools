...
- [ ] Enhancement binary property decoding — **14,276 entries, 714 names have multiple entries (stub + detailed).** Detailed entries have non-zero 0x10001A4C. Confirmed via cross-reference: 0x10001A4C = **max_ranks** (Brilliance=3, healing amp=2, stubs=0), 0x1000191A = **level/cap** (always 20 on detailed entries — may be character level cap not min req), 0x10003BE4 = **cost metric** (Brilliance=6, most=0). 0x10001804 and 0x10005E2A still unconfirmed. **NO tree membership property.** Needs: (a) filter detailed entries (non-zero 0x10001A4C), (b) confirm 0x10003BE4 meaning, (c) wire max_ranks into DB.
