...
- [ ] **PRE-FRONTEND GATE:** Schema alignment audit — verify all binary-decoded fields have corresponding DB columns and correct types; verify enum code-to-seed ID mappings are consistent; verify writer field-flow (every parser dict key consumed by insert_*); verify CHECK constraints accept all observed binary values. **Also:** review all tables for row duplication patterns (like the old bonuses table had) — check if effects, item_effects, spells, enhancements, or other tables have many similar rows that should be normalized with shared definitions + M2M junctions. Goal: ensure no data is silently dropped, rejected, or duplicated, and the schema is properly normalized.
