...
- [x] Investigate 0x47 spell entry bodies — 2,963 entries have bodies >3 bytes with 76 unique stat_def_ids in dup-triple format. Top stats: 731 (708x, small ints on damage spells — likely damage dice count), 708 (638x, small ints — likely die size/caster level), 950/947 (property refs), 946 (damage_scaling, already extracted), 553/554 (SP cost, already extracted), 1550 (timing params), 1368 (effect template refs). Bodies extend the ref list for complex spells.
- [ ] Wire spell body dup-triples — extend _overlay_spell_binary_data to scan body bytes (not just ref list) for stat dup-triples. Stats 731 and 708 may encode damage dice formulas. Currently only ref-list stats are scanned.
...
- [x] Per-field data provenance — cleaned up _WIKI_ONLY_FIELDS: removed minimum_level (now from binary), added audit verification comment. Full `_data_sources` dict deferred as over-engineering — pipeline already tracks provenance implicitly (binary runs first, wiki fills gaps). Bonuses/item_effects have explicit data_source column.
