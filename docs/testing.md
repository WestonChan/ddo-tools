...
- **Database tests are tiered — pick the vintage that matches what you're testing:**
  - **Components** that use `useDatabase`: mock the hook (`vi.mock('../hooks', ...)`) to return stub data — see [DatabaseGate.test.tsx](../src/components/DatabaseGate.test.tsx). Components never load real sql.js.
  - **Query logic** (`src/features/resources/queries/`): use `seedTestDb()` from [src/test/fixtures/resourcesDb.ts](../src/test/fixtures/resourcesDb.ts) — an in-memory sql.js DB (real WASM, loaded from node_modules) whose **schema is generated from `public/data/ddo.db`** (byte-identical CREATE statements, enforced by `schemaCompat.test.ts`) and whose **data is hand-written edge cases**, kept production-valid by the real CHECK/NOT NULL constraints. Never hand-write DDL; add new tables to `FIXTURE_TABLES`.
  - **Data-integrity literals** (hardcoded game-data strings, fixture/reality drift): use `openProjectDb()` — the real shipped DB. Use sparingly: value assertions against real data break on every DB rebuild. This tier exists because a hand-written fixture can only prove your literals match literals you also invented.
  - **Deploy compatibility** (new code vs old cached DB — the stale-service-worker incident): use `seedBaselineDb()` (frozen first-deployed schema — **never update it**) with `REQUIRED_COLUMNS` from [src/hooks/useDatabase.ts](../src/hooks/useDatabase.ts). Any new frontend query on a new DB column needs a `REQUIRED_COLUMNS` entry **in the same commit**, or `schemaCompat.test.ts` fails.
