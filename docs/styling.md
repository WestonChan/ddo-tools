...
- **Shared classes** for repeated patterns: e.g., `.sidebar-collapsible` for all text that hides on collapse.
...
| `--bg-panel` | `#27272a` | `#ffffff` | Sidebar, cards, panels |
...
.app-shell (flex column, 100vh)
  .app (CSS grid: sidebar | content | stats)
...
| `.app--sidebar-collapsed` | `56px 1fr 280px` |
| `.app--no-stats` | `220px 1fr` |
| `.app--sidebar-collapsed.app--no-stats` | `56px 1fr` |

- **Sidebar**: 220px expanded, 56px collapsed. Icon column = `--icon-col: 54px`.
...
The sidebar is always in the grid flow (never fixed-position) except at `<600px` when expanded.

| Width | Sidebar default | Expanded behavior | Notes |
|-------|----------------|-------------------|-------|
...
Key rules:
- **No media queries in App.css** -- grid columns are controlled by JS-toggled classes (`app--sidebar-collapsed`, `app--no-stats`).
- **One media query in AppSidebar.css** -- `@media (max-width: 599px)` makes `.app-sidebar.expanded` full-viewport via `position: fixed`.
